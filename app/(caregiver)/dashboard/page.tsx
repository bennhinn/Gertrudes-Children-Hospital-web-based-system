...
export default async function DashboardPage() {
  ...
  return (
    <main className="space-y-4 pb-6 lg:space-y-8 lg:pb-8">
            ...
            {nextAppointment ? (
              <>
                <Link href={`/caregiver-appointments?viewQR=${nextAppointment.id}`} className="w-full sm:w-auto sm:flex-1">
                  <Button size="lg" className="w-full bg-white text-blue-600 hover:bg-white/95 shadow-lg shadow-blue-900/20">
                ...
                <Link href="/caregiver-appointments" className="w-full sm:w-auto sm:flex-1">
                  <Button size="lg" variant="ghost" className="w-full bg-white/15 text-white backdrop-blur-md hover:bg-white/25 border border-white/20">
      ...
      {/* Billing Alert */}
      {unpaidCount > 0 && (
        <section className="rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 p-4 shadow-sm ring-1 ring-amber-200/50 sm:p-5 lg:rounded-3xl lg:p-6">
...
function StatCard({ 
  ...
  const colors = {
    blue: 'from-blue-500 to-blue-600 shadow-blue-500/25 bg-blue-50',
    purple: 'from-purple-500 to-purple-600 shadow-purple-500/25 bg-purple-50',
    emerald: 'from-emerald-500 to-emerald-600 shadow-emerald-500/25 bg-emerald-50',
    amber: 'from-amber-500 to-amber-600 shadow-amber-500/25 bg-amber-50'
  }
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-900/5 transition-all hover:shadow-md sm:p-5 lg:rounded-3xl lg:p-8">
      <div className={`absolute -right-4 -top-4 h-20 w-20 rounded-full transition-transform group-hover:scale-110 sm:-right-6 sm:-top-6 sm:h-24 sm:w-24 lg:-right-8 lg:-top-8 lg:h-32 lg:w-32 ${colors[color].split(' ').pop()}`} aria-hidden="true" />
      <div className="relative space-y-2 sm:space-y-3">
        <div className={`inline-flex h-10 w-10 items-center justify-center rounded-lg bg-gradient-to-br ${colors[color].split(' ').slice(0, 2).join(' ')} shadow-lg sm:h-12 sm:w-12 sm:rounded-xl lg:h-14 lg:w-14 lg:rounded-2xl`}>
          <Icon className="h-5 w-5 text-white sm:h-6 sm:w-6 lg:h-7 lg:w-7" />
        </div>
        <div className="space-y-0.5 lg:space-y-1">
          <p className="text-[10px] font-medium text-slate-600 sm:text-xs lg:text-sm">{label}</p>
          <p className="text-2xl font-bold text-slate-900 tracking-tight sm:text-3xl lg:text-4xl">{value}</p>
          <p className="text-[10px] text-slate-500 sm:text-xs lg:text-sm line-clamp-1">{subtext}</p>
        </div>
      </div>
    </div>
  )
...
function AppointmentRow({ appointment: apt }: { appointment: Appointment }) {
  ...
  return (
    <div className="group flex flex-col gap-3 rounded-xl bg-gradient-to-br from-slate-50 to-blue-50/20 p-3 ring-1 ring-slate-900/5 transition-all hover:shadow-md sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:rounded-2xl sm:p-4 lg:p-5">
  ...
  return (
    <div className="flex items-start gap-2.5 rounded-xl bg-white/70 p-3 backdrop-blur-sm ring-1 ring-white/50 sm:gap-3 sm:rounded-2xl sm:p-4">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[10px] text-slate-600 leading-relaxed sm:text-xs">{description}</p>
      </div>
    </div>
  )
}