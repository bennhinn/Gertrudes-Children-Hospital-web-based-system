            ...
            <header className="sticky top-0 z-50 border-b border-slate-200 bg-white shadow-sm">
                <div className="mx-auto flex h-14 sm:h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
                    <div className="flex items-center gap-3 sm:gap-4">
                        <div className="flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-lg sm:rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 text-lg sm:text-xl shadow-md">
                            👨‍⚕️
                        </div>
                        <div>
                            <h1 className="text-base sm:text-lg font-bold text-slate-800">Doctor Portal</h1>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4">
                        ...
                        <div className="hidden text-right md:block">
                            <p className="text-sm font-medium text-slate-700">Dr. {user?.fullName}</p>
                                ...
                                return (
                                    <Link
                                        ...
                                        className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all ${isActive
                                            ...
                                            : 'bg-white text-slate-700 shadow-sm hover:bg-purple-50 hover:shadow-md'
                                            }`}
                                    >
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.href}
                                href={item.href}
                                className={`flex flex-col items-center gap-0.5 px-3 py-2 min-w-[60px] ${isActive ? 'text-purple-600' : 'text-slate-500'
                                    }`}
                            >
