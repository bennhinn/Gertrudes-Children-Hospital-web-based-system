...
export default function LabLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        async function checkAuth() {
            ...
            setUser({
                fullName: user.user_metadata?.full_name || 'Lab Technician',
                email: user.email || '',
            })
            setLoading(false)
        }
            ...
            <div className="flex min-h-dvh items-center justify-center bg-gradient-to-br from-amber-50 via-yellow-50 to-lime-50">
                <div className="text-center">
                    <div className="relative mx-auto mb-6 h-16 w-16">
                        <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 opacity-20 blur-xl animate-pulse"></div>
                        <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 shadow-lg">
                            <FlaskConical className="h-8 w-8 text-white animate-pulse" />
                        </div>
                    </div>
                    <div className="space-y-2">
                        <div className="h-1.5 w-32 mx-auto rounded-full bg-amber-100 overflow-hidden">
                            <div className="h-full w-1/2 rounded-full bg-gradient-to-r from-amber-500 to-orange-500 animate-[loading_1s_ease-in-out_infinite]"></div>
                        </div>
                        <p className="text-sm text-slate-500 font-medium">Loading Laboratory...</p>
                    </div>
                </div>
            </div>
        )
                    ...
                    <nav className="flex-1 space-y-1.5 p-4">
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                ...
                                >
                                    <Icon className={`transition-all duration-200 ${isActive ? 'h-6 w-6 text-white' : 'h-5 w-5 text-slate-600 group-hover:text-amber-700 group-hover:scale-110'}`} />
                                    <span className={`flex-1 ${isActive ? 'text-white font-bold text-base' : 'text-slate-600 text-sm font-medium group-hover:text-amber-700'}`}>{item.label}</span>
